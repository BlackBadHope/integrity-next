"""One bounded I/O lifetime, adapted from the existing Agents deadline helper.

Source: integrity_agents_api/deadline.py at d6b57bed832401690628a03c78e9552c03dabc68.
Kept internal so Experience does not acquire an optional Agents dependency.
No credential lookup, ambient DNS-child environment, automatic retry or provider call.
"""
...
class DeadlineExceeded(TimeoutError):
    """A dispatched operation may be unknown and must not be replayed."""
...
# Only host/port enter this child; no SDP, provider credential or parent globals.
_DNS_CODE = (
...
class DeadlineSocket:
    """HTTPResponse facade: headers and bodies share the same raw-read budget."""
...
def bind_connection(connection, lifetime):
    """Install before request(): DNS, connect, TLS, sends and raw reads are bounded."""
