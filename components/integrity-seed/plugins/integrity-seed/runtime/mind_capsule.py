...
Seed-local copy of the Guardian ``mind_capsule`` protocol subset. The
function names, door vocabulary, link classes, and rendered text must stay
aligned with ``integrity_guardian.mind_capsule`` so extracted Seed and the
root package agree. This module does not import the monorepo root.
"""
...
_CLOCK_SPEC = importlib.util.spec_from_file_location(
    "integrity_seed_admission_clock",
    _CLOCK_PATH,
)
...
ADMISSION_TTL_HOURS = _CLOCK.ADMISSION_TTL_HOURS
admission_is_live = _CLOCK.admission_is_live
admission_clock_valid_until = _CLOCK.admission_clock_valid_until

_ADMISSION_PATH = Path(__file__).resolve().with_name("session_admission.py")
_ADMISSION_SPEC = importlib.util.spec_from_file_location(
    "integrity_seed_capsule_session_admission",
    _ADMISSION_PATH,
)
if _ADMISSION_SPEC is None or _ADMISSION_SPEC.loader is None:
    raise RuntimeError("session_admission module is missing")
_ADMISSION = importlib.util.module_from_spec(_ADMISSION_SPEC)
_ADMISSION_SPEC.loader.exec_module(_ADMISSION)
SessionAdmissionError = _ADMISSION.SessionAdmissionError
verify_session_admission = _ADMISSION.verify_session_admission
...
def build_mind_capsule(
    *,
    ...
    if admission is not None:
        ...
        except (SessionAdmissionError, ValueError, TypeError, AttributeError, KeyError):
